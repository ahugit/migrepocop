    ...
    logical, parameter :: optimize=.false.,chkstep=.true.,chkobj=.false.,condmomcompare=.false.,comparepars=.false.
...
contains
    ...
    do i=1,nl
        ...
        parname(j)='alf10' ; stepos(j)=0.8_dp ; if (i==3) stepos(j)=0.0_dp  ; if (onlyfem) stepos(j)=0.0_dp !dont iterate on alpha10(1) for a second
        j=j+1
    end do 
    ...
    do i=1,nl
        ...
        parname(j)='alf20' ; stepos(j)=0.8_dp  ; if (i==3) stepos(j)=0.0_dp ;  if (onlymales) stepos(j)=0.0_dp 
		j=j+1
	end do 
    ...
    do i=1,ntypp !The below are parameters 70 to 93. So 70-75 is type1, 76-81 is type2, 82-87 is type3, 88-93 is type4.
        if (i==1) then 
	        ...
	        alf2t(i)=realpar(j)                         ; j=j+1
            if (i<4) then
                realpar(j)= par(j)                          ; parname(j)='cst'       ; stepos(j)=2.5_dp*par(j)
                cst(i)=realpar(j)                           ; j=j+1 
            else if (i==4) then
