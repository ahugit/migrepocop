    ...
    integer(i4b), parameter :: numit=72
    ...
    real(dp), parameter :: wwage=100.0_dp,wwvar=100.0_dp,wdifww=0.01_dp,whour=100.0_dp,wcorr=1.0_dp,wwdecile=1.0_dp
    real(dp), parameter :: wrel=1.0_dp,wmove=1.0_dp,whome=1.0_dp,wkid=1.0_dp,wprop=1.0_dp		!ahu 121918 changed wmove to 10 from 1 and changed wmovemar from 10 to 100		! weights for moments for married couples. set in objfunc.
...
!    real(dp), parameter :: wwage=1.0_dp,wkid=1.0_dp,wmovemar=1.0_dp,wmovesin=1.0_dp,wwagebymove=1.0_dp		!ahu 121918 changed wmove to 10 from 1 and changed wmovemar from 10 to 100		! weights for moments for married couples. set in objfunc.
	...
	real(dp) :: cst(ntypp),kcst,divpenalty,uloc(nl),agecst,agehome
...
contains
    ...
    realpar(j) = par(j)             ; parname(j)='agehome' ; stepos(j)=0.0_dp*PAR(J)	 ; if (onlyfem) stepos(j)=0.0_dp !mult3*logit(par(2:3)) !22:23
    agehome=realpar(j)                             ; j=j+1
    realpar(j) = par(j)              ; parname(j)='agecst' ; stepos(j)=0.0_dp*PAR(J)	 ; if (onlymales) stepos(j)=0.0_dp !mult3*logit(par(2:3)) !22:23
    ...
    stepos(j)=2.0_dp            ; if (onlyfem) stepos(j)=0.0_dp ; 	stepos(j+1)=0.0_dp   ; if (onlymales) stepos(j:j+1)=0.0_dp 
    ...
    realpar(j)=par(j)                               ; parname(j)='junk' ; stepos(j)=0.0_dp   ; if (onlyfem) stepos(j)=0.0_dp  !-1.0_dp*logit(par(j)) 
    junk=realpar(j)	                            ; j=j+1
    ...
    do i=1,ntypp !The below are parameters 70 to 93. So 70-75 is type1, 76-81 is type2, 82-87 is type3, 88-93 is type4.
        if (i==1) then 
            ...
            realpar(j)=multmar * logit(par(j))          ; parname(j)='mu_mar'     ; stepos(j)=4.0_dp    ; if (onlysingles) stepos(j)=0.0_dp 	    
