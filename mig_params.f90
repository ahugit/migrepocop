...
contains
	...
	realpar(j)=par(j)               ; parname(j)='emp,cur,f' ; stepos(j)=2.2_dp ; if (onlymales) stepos(j)=0.0_dp !psio is for the offer function
    ...
    do i=1,ntypp !The below are parameters 70 to 93. So 70-75 is type1, 76-81 is type2, 82-87 is type3, 88-93 is type4.
        if (i==1) then 
            ...
            realpar(j)= par(j)                          ; parname(j)='cst'       ; stepos(j)=0.0_dp*par(j) !not iterating on this anymore. see notes. under cost vs. sigo. they are just not sep ident I think. 
            cst(i)=realpar(j)                           ; j=j+1 
	        ...
	        alf2t(i)=realpar(j)                         ; j=j+1
            if (i<4) then
                realpar(j)= par(j)                          ; parname(j)='cst'       ; stepos(j)=0.0_dp*par(j)
                cst(i)=realpar(j)                           ; j=j+1 
            else if (i==4) then
                realpar(j)= par(j)                         ; parname(j)='cst'       ; stepos(j)=0.0_dp*par(j)
                cst(i)=realpar(j)                           ; j=j+1 
