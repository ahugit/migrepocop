...
contains		
	subroutine solve
	real(dp) :: vmax(2),val(2),vsum(2),valso(2),valm(nx),valf(nx),probmuq(nx,nq,nq),probmux(nx,nx,nq) !ag090822 agsept2022
    ...
    do trueindex=1,ninp
		...
		do ia=mxa,mna,-1
			...
			if (onlysingles) then 	
                ...
                do q=1,nq
                if (ia==mxa) then 
                    vm0_c(q,x,ia,index) = utilc(1,q,x,trueindex)	! v0: value function without movecost, umar, consumption
                    vf0_c(q,x,ia,index) = utilc(2,q,x,trueindex)	! v0: value function without movecost, umar, consumption
                else 
                    vm0_c(q,x,ia,index) = utilc(1,q,x,trueindex)	+ delta * emaxm_c(q,x,ia+1)	! v0: value function without movecost, umar, consumption
                    vf0_c(q,x,ia,index) = utilc(2,q,x,trueindex)	+ delta * emaxf_c(q,x,ia+1)	! v0: value function without movecost, umar, consumption
                end if 		
                end do 
                end do 
                ...
                do q0=1,nq	
                    ...
                    vm_c=pen ; vf_c=pen ; valm=zero ; valf=zero !!ag090822 agsept2022



					prob=0.0_dp !; probmu=zero
						do q=1,nq
                            !probqu(:)=ppcq(q,q0,:) !*ppcx(x,q0,:)
                            do x=1,nx	
                                !probmu(:)=probqu(:)*ppcx(x,q0,:)
                                !ahumarch1122 
							...
							if ( ppc(q,q0) .and. maxval(qq2w(:,q0)) <= np1) then !state variable part of the q space i.e. w <= np1
                                valso=pen
								val=0.0_dp
								umar: do z=nz,1,-1		
                                    moveshocks: do iepsmove=1,nepsmove
                                        !ag090122 agsept2022 dd = (/ia,trueindex,q,x,z,q0,gender/callfrom,-1,-1,-1,iepsmove /) 	
                                                                 !(/ia,trueindex,q,x,z,q0,gender/callfrom,jmax,qmax,relmax,iepsmove /)  	
                                ...
                                valm(:)=valm(:)+probmuq(:,q,q0)*probmux(:,x,q0)*val(1)
                                valf(:)=valf(:)+probmuq(:,q,q0)*probmux(:,x,q0)*val(2)
                                !valm(:)=valm(:)+probmu(:)*val(1) !ag090822 agsept2022 val=val+ppso(iepsmove)*mgt(z)*vmax
                                !valf(:)=valf(:)+probmu(:)*val(2) !ag090822 agsept2022 val=val+ppso(iepsmove)*mgt(z)*vmax
								!!ag090822 agsept2022 
                                !vm_c(q,x)=val(1) ; vf_c(q,x)=val(2)
                                !do x0=1,nx
                                !valm(x0)=valm(x0)+ppcq(q,q0,x0)*ppcx(x,q0,x0)*val(1) !ag090822 agsept2022 val=val+ppso(iepsmove)*mgt(z)*vmax
                                !valf(x0)=valf(x0)+ppcq(q,q0,x0)*ppcx(x,q0,x0)*val(2) !ag090822 agsept2022 val=val+ppso(iepsmove)*mgt(z)*vmax
                                !end do !x0
							end if !pc0(q0)=.true. i.e. w(1) or w(2) are not np2 and pc
						end do !x !ag090822 agsept2022
					end do !q !ag090822 agsept2022
					!if (icheck_eqvmvf) then
					!	call check_eqvmvf(dd,vm_c,vf_c) 
					!end if 
                    !if (Trueindex==1) print*, "maybe this much is enough I can't do it"
                    do x0=1,nx
                    emaxm_c(q0,x0,ia)=valm(x0)
                    emaxf_c(q0,x0,ia)=valf(x0)
                    end do 
                    !!!ag090822 agsept2022 do x0=1,nx
                    !!!ag090822 agsept2022     if ( maxval(qq2w(:,q0)) <= np1 ) then !state variable part of the q space i.e. w <= np1
						    !prob=matmul( reshape(ppcq(:,q0,x0),(/nq,1/)) , reshape(ppcx(:,q0,x0),(/1,nx/)) )
						    !emaxm_c(q0,x0,ia)=sum(prob*vm_c)
						    !emaxf_c(q0,x0,ia)=sum(prob*vf_c)                    
                    !!!ag090822 agsept2022 emaxm_c(q0,x0,ia)=0.0_dp
					!!!ag090822 agsept2022 emaxf_c(q0,x0,ia)=0.0_dp
       			    !!!ag090822 agsept2022 do q=1,nq
                    !!!ag090822 agsept2022 do x=1,nx
					!!!ag090822 agsept2022 emaxm_c(q0,x0,ia)=emaxm_c(q0,x0,ia)+ppcq(q,q0,x0)*ppcx(x,q0,x0)*vm_c(q,x)
					!!!ag090822 agsept2022 emaxf_c(q0,x0,ia)=emaxf_c(q0,x0,ia)+ppcq(q,q0,x0)*ppcx(x,q0,x0)*vf_c(q,x)
                    !!!ag090822 agsept2022 end do 
                    !!!ag090822 agsept2022 end do 
                    !!!ag090822 agsept2022 end if !state variable part of the q space i.e. w <= np1
					!!!ag090822 agsept2022 end do	 !x0						
...
! if (Trueindex==1) print*, "maybe this much is enough I can't do it q0 edition"
    ...
    subroutine getdec(dd,vmax,vsing) 
        ...
        if (Callfrom==40) then !calling from couples
            iepsmove=dd(11)   
            vec=pen
